...
@endsection
@section('js')
    <script src="{{asset('resources/assets/pages/scripts/table-datatables-managed.js')}}" type="text/javascript"></script>
    <script>
        var token = "{{csrf_token()}}";

        function doDelete(doc_id) {
            //删除管理员
            bootbox.confirm("是否确定删除该条记录?", function (result) {
                if (result) {
                    $.post("{{url('admin/doctor/')}}/" + doc_id, {
                        '_method': 'delete',
                        '_token': token
                    }, function (data) {
